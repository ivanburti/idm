...
class UserTable
{
    ...
    private function convert_to_int(&$value, $key)
    {
        $value = (int) $value;
    }
    ...
    public function getUsersByOrganizationId($organization_id)
    {
        ...
        array_walk($organization_id, [$this, 'convert_to_int']);
        return $this->tableGateway->select(['organization_organization_id' => $organization_id]);
    }
    ...
    public function getEmployees($onlyEnabled = true)
    {
        ...
        $select->where->isNull('organization.is_external');
    ...
    {
        $select = $this->getSelectUsers();
        $select->where->isNotNull('organization.is_external');
    ...
    */
    ...
    public function saveUser(User $user)
    {
        ...
        if (! $this->getUserById($user_id)) {
            throw new RuntimeException(sprintf('Cannot update user with id %d; does not exist', $user_id));
        }
